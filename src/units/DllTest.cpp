...
void DllTest::testEchoStrategy()
{
	...
	ON_INITIALIZE_CALLBACK fpInit =static_cast<ON_INITIALIZE_CALLBACK>(library.getSymbol("onStart"));
	ON_TICK_CALLBACK fpOnTick = static_cast<ON_TICK_CALLBACK>(library.getSymbol("onTickIn"));
	ON_UNINITIALIZE_CALLBACK fpUninit = static_cast<ON_UNINITIALIZE_CALLBACK>(library.getSymbol("onStop"));
...
void DllTest::testStrategy1()
{
	...
	ON_INITIALIZE_CALLBACK fpInit = static_cast<ON_INITIALIZE_CALLBACK>(library.getSymbol("onInitialize"));
	ON_TICK_CALLBACK fpOnTick = static_cast<ON_TICK_CALLBACK>(library.getSymbol("onTick"));
	ON_UNINITIALIZE_CALLBACK fpUninit = static_cast<ON_UNINITIALIZE_CALLBACK>(library.getSymbol("onUninitialize"));
	std::string hello = "Hello world!";
	fpInit();
	fpOnTick(hello.c_str(), hello.length());
	fpUninit();
